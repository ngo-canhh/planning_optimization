...
class LocalSearchSolver:
    def __init__(self,
                ...
                max_swaps_per_op_cap=5,
                verbose=False  # Tắt verbose để không in debug info
                ):
    ...
    def _create_initial_solution(self):
        ...
        for d in range(self.D):
            # If worked night shift yesterday -> off today
            if d > 0:
                for i in range(self.N):
    ...
    def _calculate_fitness(self, individual):
        ...
        # Calculate max night shifts per staff
    ...
    def _swap_night_with_off_neighbor(self, current_solution, num_swaps_to_perform=1):
        ...
        for _ in range(num_swaps_to_perform):
            ...
            for _ in range(max_attempts_one_swap):
                if self.N < 2 and num_swaps_to_perform > 0:
                    return neighbor
    ...
    def _fix_night_shift_violation_neighbor(self, current_solution):
        ...
        neighbor[n_violating][d_violating_work] = 0
        return neighbor
    
    def _generate_neighbor(self, current_solution, num_swaps_for_swap_op=1):
        operators = [
            (self._swap_night_with_off_neighbor, 0.70),
    ...
    def _local_search(self, initial_solution_func, fitness_func, neighbor_func, 
                     max_iterations, max_iterations_no_improvement, time_limit_for_run_secs=None):
...
def main():
    ...
    pre_assigned_off_data = []
    
    for i in range(N):
        days = list(map(int, input().split()))
        pre_assigned_off_data.append(days)
    
    # Giải bài toán
    solver = LocalSearchSolver(verbose=False)
    solution, fitness, run_time = solver.solve(
        N=N, 
        D=D, 
        A=A, 
        B=B,
        pre_assigned_off_list_of_lists=pre_assigned_off_data,
        time_limit_for_solve_secs=60
    )
    ...
    if solution:
        ...
        for i in range(N):
            row = []
            for d in range(D):
                if pre_assigned_off_data[i] and (d+1) in pre_assigned_off_data[i]:
                    row.append("0")
                else:
